...
// Declaración global para que TypeScript reconozca la librería de PayPhone
declare global {
  interface Window {
    payphone: any;
  }
...
const ClientCasesView: React.FC<{ session: Session }> = ({ session }) => {
    ...
    // Refs para evitar el problema de "stale closures" con el SDK de PayPhone
    const payAmountRef = useRef(payAmount);
    const payDescRef = useRef(payDesc);

    useEffect(() => { payAmountRef.current = payAmount; }, [payAmount]);
    useEffect(() => { payDescRef.current = payDesc; }, [payDesc]);

    // LOGICA DE INTEGRACION PAYPHONE
    useEffect(() => {
        if (payMode === 'payphone') {
            const initPayPhone = () => {
                if (window.payphone) {
                    ...
                    window.payphone.Button({
                        token: (import.meta as any).env.VITE_PAYPHONE_TOKEN || '', 
                        btnHorizontal: true,
                        btnDark: true,
                        createOrder: function(actions: any) {
                            const amt = parseFloat(payAmountRef.current);
                            if (isNaN(amt) || amt <= 0) {
                                alert("Por favor ingresa un monto válido mayor a 0 antes de continuar con el pago.");
                                return false;
                            }
                            const amtInCents = Math.round(amt * 100); 
                            return actions.prepare({
                                amount: amtInCents,
                                amountWithoutTax: amtInCents,
                                currency: 'USD',
    ...
    const handlePayPhoneSuccess = async (model: any) => {
        ...
        const { error } = await supabase.from('pagos').insert({
            ...
            motivo_rechazo: `Aprobado por PayPhone Transacción: ${model.transactionId}`
        });
        ...
        if (!error) {
            ...
            alert('El cobro se realizó, pero falló el registro interno: ' + error.message);
        }
            ...
            <Modal isOpen={!!payMode} onClose={() => { setPayMode(null); setSelectedUpdateForPay(null); setAbonarCase(null); setCustomAmount(''); setPayAmount(''); }}>
                <div className="p-8 flex flex-col max-h-[85vh]">
                    ...
                    <div className="flex gap-2 mb-6">
                        <button type="button" onClick={() => setPayMode('transferencia')} className={`flex-1 py-3 rounded-xl transition-all font-bold text-xs tracking-widest uppercase ${payMode === 'transferencia' ? 'bg-blue-600/80 text-white shadow-lg border border-blue-500/50' : 'bg-white/5 text-zinc-400 hover:text-white border border-transparent'}`}>
                            TRANSFERENCIA
                        </button>
                        <button type="button" onClick={() => setPayMode('payphone')} className={`flex-1 py-3 rounded-xl transition-all font-bold text-xs tracking-widest uppercase ${payMode === 'payphone' ? 'bg-orange-600/80 text-white shadow-lg border border-orange-500/50' : 'bg-white/5 text-zinc-400 hover:text-white border border-transparent'}`}>
                            PAYPHONE (TARJETA)
                        </button>
                    </div>
                    ...
                    <div className={`overflow-y-auto pr-2 ${scrollbarStyle}`}>
                        {payMode === 'payphone' ? (
                            <div className="flex flex-col items-center justify-center py-4">
                                ...
                                <p className="text-zinc-400 text-center text-xs mb-8">Paga con tarjeta de crédito o débito de forma rápida.</p>
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center gap-4 mt-2">
                                            ...
                                            <button type="button" onClick={() => fileInputRef.current?.click()} className={`p-4 border rounded-xl transition-colors ${payFile ? 'text-blue-500 border-blue-900 bg-blue-900/20' : 'text-zinc-400 border-white/10 hover:text-white hover:bg-white/5'}`}>
                                                <DocumentIcon />
                                            </button>
...
const ClientDashboard: React.FC<{ session: Session }> = ({ session }) => {
    ...
    const checkInitialData = useCallback(async () => {
        ...
        if (profile) {
            ...
            setCedulaInput(profile.cedula || '');
            setNombreInput(profile.primer_nombre || '');
            setApellidoInput(profile.primer_apellido || '');

            if (profile.cambiar_pass_obligatorio === true) {
                setShowPasswordModal(true);
            } else if (!profile.cedula || !profile.primer_nombre || !profile.primer_apellido || profile.perfil_completo === false) {
        ...
        if (myCases && myCases.length > 0) {
            ...
            const [times, exps, payments, updates] = await Promise.all([
                supabase.from('time_entries').select('*').in('caso_id', caseIds).order('fecha_tarea', { ascending: false }),
                supabase.from('gastos').select('*').in('caso_id', caseIds).order('fecha', { ascending: false }),
                supabase.from('pagos').select('*').in('caso_id', caseIds).order('created_at', { ascending: false }),
                supabase.from('case_updates').select('id, descripcion, created_at, caso:cases(titulo)').in('case_id', caseIds).eq('estado_aprobacion', 'aprobado').order('created_at', { ascending: false }).limit(10)
            ]);

            setTimeEntries(times.data || []);
            setExpenses(exps.data || []);
            setCasePagos(payments.data || []);
            setNotifications(updates.data || []);
        }
    }, [session.user.id]);

    useEffect(() => { checkInitialData(); }, [checkInitialData]);

    const handleSaveRequiredData = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!cedulaInput.trim() || !nombreInput.trim() || !apellidoInput.trim()) return;
        setDataLoading(true);
        const { error } = await supabase.from('profiles').update({ cedula: cedulaInput.trim(), primer_nombre: nombreInput.trim(), primer_apellido: apellidoInput.trim(), perfil_completo: true }).eq('id', session.user.id);
        if (!error) { setProfileIncomplete(false); checkInitialData(); } 
    ...
    const handleUpdatePassword = async (e: React.FormEvent) => {
        ...
        try {
            ...
            checkInitialData();
        } catch (error: any) { alert(`Error: ${error.message}`); } finally { setPassLoading(false); }
    };
    ...
    const handleMenuClick = (view: string) => {
        if (profileIncomplete) { alert("Por favor, actualiza tus datos obligatorios."); return; }
        setActiveView(view);
        closeAllMenus();
    };
        ...
        <Fragment>
            ...
            <div className="bg-black min-h-screen text-white flex flex-col font-sans relative">
                                ...
                                <>
                                    ...
                                    <div className="absolute right-0 mt-6 w-64 bg-black/60 backdrop-blur-xl border border-white/10 shadow-2xl shadow-black rounded-2xl py-2 z-[100] animate-in fade-in slide-in-from-top-3 duration-300 overflow-hidden font-mono">
                                        <div className="p-5 border-b border-white/5">
                                            ...
                                            <p className="text-sm font-bold text-white truncate mb-1">{clientProfile?.primer_nombre && clientProfile?.primer_apellido ? `${clientProfile.primer_nombre} ${clientProfile.primer_apellido}` : session.user.email}</p>
                                            <p className="text-[10px] text-blue-400 uppercase tracking-[0.2em] font-black">Cliente</p>
                                        </div>
                                        <button onClick={() => { handleMenuClick('PROFILE'); setProfileMenuOpen(false); }} className="w-full text-left px-5 py-4 text-xs font-bold uppercase tracking-widest text-zinc-300 hover:text-white hover:bg-white/10 transition-colors border-b border-white/5 disabled:opacity-50" disabled={profileIncomplete}>
                                            Mi Perfil & Pagos
                                        </button>
                                        <button onClick={async () => { closeAllMenus(); localStorage.removeItem('deviceToken'); await supabase.auth.signOut(); }} className="w-full text-left px-5 py-4 text-xs font-bold uppercase tracking-widest text-red-400 hover:text-red-300 hover:bg-red-900/20 transition-colors">
                                            Cerrar Sesión
                                        </button>
                                    </div>
                                </>
                ...
                <main className="flex-grow flex flex-col relative z-10">
                    <div className={`absolute inset-0 flex items-center justify-center pointer-events-none transition-opacity duration-500 ${activeView === 'HOME' && !profileIncomplete ? 'opacity-100 z-10' : 'opacity-0 -z-10 hidden'}`}>
                        <div className="text-center font-bold text-5xl md:text-7xl relative w-full flex items-center justify-center font-serif tracking-widest animate-in fade-in duration-1000 slide-in-from-bottom-8">
                            <h1 className="text-white drop-shadow-2xl px-4">Regalado & Regalado</h1>
                        </div>
                    </div>

                    <div className={`p-4 sm:p-8 w-full max-w-7xl mx-auto flex-grow flex-col ${activeView === 'HOME' && !profileIncomplete ? 'hidden' : 'flex'}`}>
                        
                        {/* PANTALLA FORZADA: ACTUALIZACIÓN DE DATOS */}
                        <div className={`${profileIncomplete ? 'flex-grow flex items-center justify-center p-4' : 'hidden'}`}>
                            <form onSubmit={handleSaveRequiredData} className="bg-black/60 backdrop-blur-xl border border-white/10 w-full max-w-lg p-10 font-mono shadow-2xl rounded-2xl animate-in fade-in zoom-in-95 duration-500">
                                <LockClosedIcon className="w-16 h-16 text-yellow-500 mx-auto mb-6 p-4 bg-yellow-950/30 rounded-full border-2 border-yellow-800" />
                                <h1 className="text-lg font-black tracking-[0.2em] uppercase text-white mb-2 text-center">
                                    REGALADO Y REGALADO
                                </h1>
                                <p className="text-zinc-500 text-xs text-center uppercase tracking-widest font-bold border-b border-zinc-900 pb-4 mb-8">
                                    Actualiza tus datos para un mejor servicio
                                </p>
                                
                                <div className="space-y-6 mb-10">
                                    <div>
                                        <label className="block text-zinc-500 text-[10px] font-black mb-2 uppercase tracking-[0.3em]">CÉDULA DE IDENTIDAD / RUC</label>
                                        <input type="text" required value={cedulaInput} onChange={e => setCedulaInput(e.target.value)} className="w-full bg-transparent border-b-2 border-white/20 text-white pb-2 focus:outline-none focus:border-white transition-colors text-lg tracking-wider" placeholder="17xxxxxxxxxx" />
                                    </div>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <label className="block text-zinc-500 text-[10px] font-black mb-2 uppercase tracking-[0.3em]">PRIMER NOMBRE</label>
                                            <input type="text" required value={nombreInput} onChange={e => setNombreInput(e.target.value)} className="w-full bg-transparent border-b-2 border-white/20 text-white pb-2 focus:outline-none focus:border-white transition-colors uppercase tracking-wider" placeholder="Ej: Juan" />
                                        </div>
                                        <div>
                                            <label className="block text-zinc-500 text-[10px] font-black mb-2 uppercase tracking-[0.3em]">PRIMER APELLIDO</label>
                                            <input type="text" required value={apellidoInput} onChange={e => setApellidoInput(e.target.value)} className="w-full bg-transparent border-b-2 border-white/20 text-white pb-2 focus:outline-none focus:border-white transition-colors uppercase tracking-wider" placeholder="Ej: Perez" />
                                        </div>
                                    </div>
                                </div>

                                {cedulaInput.trim() && nombreInput.trim() && apellidoInput.trim() && (
                                     <div className="flex justify-end">
                                        <button type="submit" disabled={dataLoading} className="bg-gradient-to-tr from-green-500 to-green-400 text-black font-black py-4 px-10 rounded-xl transition-all shadow-lg shadow-green-500/20 transform hover:scale-[1.02] disabled:opacity-50 text-xs tracking-widest uppercase">
                                            {dataLoading ? 'PROCESANDO...' : 'GUARDAR Y CONTINUAR'}
                                        </button>
                                    </div>
                                )}

                                {(!cedulaInput.trim() || !nombreInput.trim() || !apellidoInput.trim()) && (
                                    <div className="flex gap-4 items-center bg-black/40 border-t border-white/10 p-5 mt-10 rounded-b-2xl -mx-10 -mb-10 opacity-70">
                                        <UserIcon className="w-10 h-10 text-zinc-700 pointer-events-none" />
                                        <div className="flex-grow">
                                            <p className="font-serif font-bold text-2xl tracking-widest text-zinc-700">R & R</p>
                                            <p className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold mt-1">Completa los campos para activar el dashboard</p>
                                        </div>
                                    </div>
                                )}
                            </form>
                        ...
                        {/* VISTAS NORMALES OCULTAS SI EL PERFIL ESTÁ INCOMPLETO */}
                ...
                <Modal isOpen={showPasswordModal} preventClose={true}>
                    <form onSubmit={handleUpdatePassword} className="bg-black/60 backdrop-blur-xl border border-white/10 w-full max-w-md mx-auto p-10 font-serif shadow-2xl rounded-2xl">
                        <LockClosedIcon />
                        <h2 className="text-sm font-bold tracking-[0.3em] uppercase text-yellow-500 mb-8 text-center">
                            Actualización Obligatoria de Contraseña
                        </h2>
                        <input type="password" required value={newPassword} onChange={e => setNewPassword(e.target.value)} className="w-full bg-transparent border-b-2 border-white/20 text-white pb-2 focus:outline-none focus:border-white transition-colors mb-8 font-sans text-lg tracking-wider" placeholder="Nueva contraseña maestra..."/>
                        
                        <div className="flex justify-end font-sans">
                            <button type="submit" disabled={passLoading || newPassword.length < 8} className="bg-white text-black font-bold py-3 px-8 rounded-xl text-xs tracking-widest uppercase disabled:opacity-50">
                                {passLoading ? 'GUARDANDO...' : 'GUARDAR Y CONTINUAR'}
                            </button>
                        </div>
                    </form>
                </Modal>
