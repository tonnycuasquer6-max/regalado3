    ...
    return (
        <div className="bg-black min-h-screen text-white flex flex-col font-mono relative">
            ...
            <main className="flex-grow flex flex-col relative z-10">
                <div className={`absolute inset-0 flex items-center justify-center pointer-events-none transition-opacity duration-500 ${activeView === 'HOME' ? 'opacity-100' : 'opacity-0 hidden'}`}>
                    <div className="text-center font-black text-6xl relative h-20 w-full flex items-center justify-center">
                        <h1 className="absolute transition-all duration-1000 ease-in-out tracking-[.2em] text-white">
                            Regalado & Regalado
                        </h1>
                    </div>
                </div>
                ...
                <div className={`p-4 sm:p-8 w-full max-w-7xl mx-auto flex-grow flex-col ${activeView === 'HOME' ? 'hidden' : 'flex'}`}>
                    <div className={activeView === 'CLIENTS' ? 'flex-grow flex flex-col h-full' : 'hidden'}><WorkerClientsView session={session} userRole={userRole} /></div>
                    <div className={activeView === 'ASSIGNED_CASES' ? 'flex-grow flex flex-col h-full' : 'hidden'}><WorkerAssignedCasesView session={session} /></div>
                    <div className={activeView === 'TIME_BILLING' ? 'flex-grow flex flex-col h-full' : 'hidden'}><TimeBillingView session={session} onCancel={() => handleMenuClick('HOME')} /></div>
                    <div className={activeView === 'EXPENSES' ? 'flex-grow flex flex-col h-full' : 'hidden'}><ExpensesView /></div>
                    <div className={activeView === 'CHAT' ? 'flex-grow flex flex-col h-full' : 'hidden'}><WorkerChatView session={session} /></div>
                    <div className={activeView === 'PROFILE' ? 'flex-grow flex flex-col h-full' : 'hidden'}><WorkerProfile session={session} onCancel={() => handleMenuClick('HOME')} /></div>
                </div>
            </main>
        </div>
    );
