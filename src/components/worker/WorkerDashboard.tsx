...
const Modal: React.FC<{ isOpen: boolean; onClose: () => void; children: React.ReactNode }> = ({ isOpen, onClose, children }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[60] flex items-center justify-center p-4 font-mono">
            <div className="bg-black border border-zinc-800 shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col relative">
                {/* Botón de cierre en la esquina superior derecha */}
                <button onClick={onClose} className="absolute top-4 right-4 text-zinc-500 hover:text-white z-50 transition-colors">
                    <CloseIcon />
                </button>
                {children}
            </div>
...
const WorkerClientsView: React.FC<{ session: Session }> = ({ session }) => {
    ...
    const [newClientData, setNewClientData] = useState({ primer_nombre: '', segundo_nombre: '', primer_apellido: '', segundo_apellido: '', cedula: '' });
    const photoInputRef = useRef<HTMLInputElement>(null);

    const fetchData = useCallback(async () => {
        setLoading(true);
        // CORRECCIÓN 1: Buscar correctamente a los clientes para que NO salga vacío
        ...
        // Obtener asignaciones reales para el auto-desbloqueo
    ...
    const handleCreateClient = async (e: React.FormEvent) => {
        ...
        const { error } = await supabase.from('profiles').insert({
            ...newClientData,
            email: `${Date.now()}@pendiente.com`, // Email temporal oculto requerido por DB
            rol: 'cliente',
        ...
        if (error) alert(error.message);
            ...
            setNewClientData({ primer_nombre: '', segundo_nombre: '', primer_apellido: '', segundo_apellido: '', cedula: '' });
            await fetchData();
        }
        setActionLoading(false);
    };
    ...
    return (
        <div className="animate-in fade-in duration-500">
            ...
            {/* CORRECCIÓN FORMULARIO CLIENTE: Scroll, Sin Email, Botón Foto Funcional */}
                ...
                <form onSubmit={handleCreateClient} className="bg-black w-full text-white font-mono flex flex-col max-h-[85vh]">
                    ...
                    <div className={`p-8 pt-4 overflow-y-auto flex-grow ${scrollbarStyle}`}>
                        <div className="flex flex-col items-center mb-10">
                            ...
                            {/* Este botón ahora SÍ abre el explorador de archivos */}
                    ...
                    <div className="p-8 pt-4 border-t border-zinc-900 flex justify-end flex-shrink-0 bg-black">
                        <button type="submit" disabled={actionLoading} className="bg-white text-black font-bold py-3 px-8 text-[10px] tracking-widest uppercase hover:bg-zinc-300 transition-colors disabled:opacity-50 w-full md:w-auto text-center">
                            ENVIAR A REVISIÓN
                        </button>
                    </div>
                </form>
...
// ==========================================
// VISTA 2: CASOS ASIGNADOS (CASCADA + HISTORIAL)
// ==========================================
    ...
    return (
        <div className="bg-black min-h-screen text-white flex flex-col font-mono relative">
            ...
            <header className="flex justify-between items-center p-6 bg-black sticky top-0 z-50">
                ...
                {/* CORRECCIÓN MENÚ: Centrado, letras grandes, sin líneas inferiores */}
                ...
                <div className="flex items-center justify-end gap-6 w-32 relative">
                    ...
                    <div className="relative">
                        ...
                        {/* CORRECCIÓN MENÚ PERFIL: La capa negra transparente fuerza el cierre al dar clic fuera o en una opción */}
                        {profileMenuOpen && (
                            <>
