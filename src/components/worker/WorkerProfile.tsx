...
const Modal: React.FC<{ isOpen: boolean; onClose: () => void; children: React.ReactNode }> = ({ isOpen, onClose, children }) => {
    ...
    return createPortal(
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[60] flex items-center justify-center p-4 font-mono">
            <div className="bg-black border border-zinc-800 shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh] relative rounded-2xl">
                {children}
            </div>
        </div>,
        document.body
...
const WorkerProfile: React.FC<{ session: Session; onCancel: () => void }> = ({ session, onCancel }) => {
    ...
    // MATEMÁTICAS SEPARADAS PARA RESUMEN GLOBAL
    ...
    timeEntries.forEach(te => {
        const amt = (te.horas || 0) * (te.tarifa_personalizada || 0);
        if (te.estado === 'cobrado' && te.estado_pago_contador === 'aprobado') {
            totalConfirmedIncome += amt;
        } else {
            totalPendingIncome += amt;
        }
    });

    const monthName = currentMonth.toLocaleDateString('es-ES', { month: 'long', year: 'numeric' }).toUpperCase();

    // MATEMÁTICAS SEPARADAS POR CLIENTE CON EL FIX ANY
    const clientsWithActivity = Object.values(clientsDict).map((client: any) => {
        ...
        return { client, cTime, cExp, cTotalHrs, cTotalPending, cTotalConfirmed, cTotalExp };
    }).filter(g => g.cTime.length > 0 || g.cExp.length > 0);

    const clientOptions = Object.values(clientsDict);
    // CON EL FIX ANY AQUI TAMBIEN
    ...
    return (
        <Fragment>
            ...
            <div className="max-w-5xl mx-auto animate-in fade-in duration-500 font-mono text-white pb-12 relative w-full">
                ...
                {profileData && (
                    <div className="bg-zinc-950 border border-zinc-900 p-8 flex flex-col md:flex-row items-center gap-8 mb-8 shadow-2xl shadow-black relative overflow-hidden rounded-2xl">
                        ...
                        <div className="text-center md:text-left z-10">
                            ...
                            <p className="text-zinc-500 uppercase tracking-widest mt-1 text-sm">{profileData.rol} | {profileData.categoria_usuario}</p>
                            <div className="mt-4 flex flex-col gap-1 text-xs text-zinc-400 font-mono">
                                ...
                                {profileData.categoria_usuario === 'abogado' && (
                ...
                <div className="flex justify-end gap-4 mb-6">
                    <button onClick={() => setIsExpenseModalOpen(true)} className="bg-white text-black font-bold py-2 px-6 text-[10px] tracking-widest uppercase hover:bg-zinc-300 transition-colors shadow-lg shadow-black/50 rounded-xl">
                        + AÑADIR GASTO
                    </button>
                    <button onClick={() => setShowRecords(!showRecords)} className="bg-zinc-800 text-white font-bold py-2 px-6 text-[10px] tracking-widest uppercase hover:bg-zinc-700 transition-colors shadow-lg shadow-black/50 rounded-xl">
                        {showRecords ? 'OCULTAR REGISTROS' : 'VER REGISTROS'}
                    </button>
                </div>

                <div className="bg-black border border-zinc-800 shadow-2xl shadow-black/50 p-8 rounded-2xl">
                    ...
                    {loading ? (
                        ...
                        <div>
                            ...
                            <div className="bg-zinc-950 border border-zinc-900 p-6 mb-8 rounded-xl">
                                ...
                                <div className="border-t-2 border-dashed border-zinc-800 pt-6 mt-4 flex justify-between items-center">
                                    ...
                                    <p className="text-yellow-500 font-black text-xl tracking-wider">${totalPendingIncome.toFixed(2)}</p>
                                </div>
                                <div className="border-t-2 border-dashed border-zinc-800 pt-4 mt-4 flex justify-between items-center">
                                    ...
                                    <p className="text-green-500 font-black text-3xl tracking-wider">${totalConfirmedIncome.toFixed(2)}</p>
                                </div>
                            </div>
                            ...
                            {showRecords && (
                                <div className="mb-8 animate-in fade-in slide-in-from-top-4 duration-500">
                                    ...
                                    {clientsWithActivity.length === 0 ? (
                                        <div className="p-8 border border-dashed border-zinc-900 text-center text-zinc-600 text-xs tracking-widest uppercase rounded-xl">
                                            No hay actividad en este mes.
                                        </div>
                                    ) : (
                                        <div className="space-y-4">
                                            {/* CUARTO FIX ANY EN EL MAP */}
                                            {clientsWithActivity.map(({ client, cTime, cExp, cTotalHrs, cTotalPending, cTotalConfirmed, cTotalExp }: any) => (
                                                <div key={client.id} className="bg-zinc-950 border border-zinc-900 transition-colors hover:border-zinc-700 rounded-xl overflow-hidden">
                                                    ...
                                                    <div className="p-4 flex flex-col md:flex-row justify-between md:items-center gap-4">
                                                        <div className="flex items-center gap-4 flex-grow">
                                                            ...
                                                            <div>
                                                                ...
                                                                <div className="flex flex-wrap gap-2 mt-2">
                                                                    {cTime.length > 0 && (
                                                                        <button onClick={() => toggleClientTab(client.id, 'time')} className={`text-[9px] font-bold uppercase tracking-widest px-3 py-1 border transition-colors rounded ${activeClientTab.id === client.id && activeClientTab.tab === 'time' ? 'bg-white text-black border-white' : 'bg-transparent text-zinc-400 border-zinc-700 hover:text-white'}`}>
                                                                            Ver Registros ({cTime.length})
                                                                        </button>
                                                                    )}
                                                                    {cExp.length > 0 && (
                                                                        <button onClick={() => toggleClientTab(client.id, 'expenses')} className={`text-[9px] font-bold uppercase tracking-widest px-3 py-1 border transition-colors rounded ${activeClientTab.id === client.id && activeClientTab.tab === 'expenses' ? 'bg-white text-black border-white' : 'bg-transparent text-zinc-400 border-zinc-700 hover:text-white'}`}>
                                                        ...
                                                        <div className="flex gap-6 text-right w-full md:w-auto justify-end border-t md:border-t-0 border-zinc-900 pt-4 md:pt-0">
                                                            ...
                                                            <div>
                                                                <p className="text-[9px] text-yellow-600 uppercase tracking-widest font-black mb-1">P. CONFIRMAR</p>
                                                                <p className="text-yellow-500 font-mono font-bold">${cTotalPending.toFixed(2)}</p>
                                                            </div>
                                                            <div>
                                                                <p className="text-[9px] text-green-600 uppercase tracking-widest font-black mb-1">COBRADO</p>
                                                                <p className="text-green-500 font-mono font-bold">${cTotalConfirmed.toFixed(2)}</p>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    ...
                                                    {activeClientTab.id === client.id && activeClientTab.tab === 'time' && (
                                                        <div className="bg-black border-t border-zinc-900 p-4 space-y-2">
                                                            {cTime.map((te: any) => {
                                                                ...
                                                                const borderColor = isConfirmed ? 'border-green-900/50' : isRejected ? 'border-red-900/50' : 'border-yellow-900/50';
                                                                const barColor = isConfirmed ? 'bg-green-500' : isRejected ? 'bg-red-500' : 'bg-yellow-500';

                                                                return (
                                                                    <div key={te.id} className={`bg-zinc-950 border ${borderColor} p-4 rounded-xl flex flex-col md:flex-row justify-between md:items-center gap-4 relative overflow-hidden`}>
                                                                        <div className={`absolute top-0 left-0 w-1 h-full ${barColor}`}></div>
                                                                        <div className="flex-grow grid grid-cols-1 md:grid-cols-2 gap-4 pl-2">
                                                                            <div>
                                                                        ...
                                                                        <div className="flex-shrink-0 text-right md:border-l border-zinc-800 md:pl-6 pt-2 md:pt-0 border-t md:border-t-0">
                                                                            <p className="text-[9px] text-zinc-500 uppercase tracking-widest font-black mb-1">ESTADO PAGO</p>
                                                                            {isConfirmed ? (
                                                                                <p className="text-green-500 text-xl font-black tracking-wider flex items-center justify-end gap-1">
                                                    ...
                                                    {activeClientTab.id === client.id && activeClientTab.tab === 'expenses' && (
                                                        <div className="bg-black border-t border-zinc-900 p-4 space-y-2">
                                                            {cExp.map((exp: any) => {
                                                                ...
                                                                return (
                                                                    <div key={exp.id} className="flex flex-col md:flex-row justify-between md:items-center p-4 rounded-xl bg-zinc-950 border border-zinc-900 gap-4">
                                                                        <div>
                                                                        ...
                                                                        <div className="flex items-center gap-6 justify-between md:justify-end border-t md:border-t-0 border-zinc-800 pt-3 md:pt-0">
                                                                            {receiptUrl && receiptUrl.trim() !== '' && (
                                                                                <a href={receiptUrl} target="_blank" rel="noreferrer" className="text-blue-400 hover:text-blue-300 text-[10px] tracking-widest uppercase flex items-center gap-1 rounded border border-blue-900/50 bg-blue-950/30 px-2 py-1">
                                                                                    <DocumentIcon /> Recibo
                                                                                </a>
                                                                            )}
                ...
                <Modal isOpen={isExpenseModalOpen} onClose={() => setIsExpenseModalOpen(false)}>
                    <form onSubmit={handleSaveExpense} className="p-8 flex flex-col overflow-y-auto max-h-[90vh]">
                            ...
                            <div className="grid grid-cols-2 gap-6 items-start">
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-4 mt-2">
                                        ...
                                        <button type="button" onClick={() => fileInputRef.current?.click()} className={`p-3 border border-zinc-800 rounded-xl transition-colors ${expFile ? 'text-green-500 border-green-900 bg-green-950/30' : 'text-zinc-500 hover:text-white hover:bg-zinc-900'}`}>
                                            <PaperClipIcon />
                                        </button>
                        ...
                        <div className="flex justify-end gap-4 border-t border-zinc-900 pt-6">
                            <button type="button" onClick={() => setIsExpenseModalOpen(false)} className="text-zinc-500 text-[10px] font-bold tracking-widest hover:text-white uppercase transition-colors border border-zinc-800 rounded-xl px-6 py-2">CANCELAR</button>
                            <button type="submit" disabled={actionLoading} className="bg-white text-black font-bold py-2 px-6 rounded-xl text-[10px] tracking-widest uppercase hover:bg-zinc-300 transition-colors disabled:opacity-50">GUARDAR GASTO</button>
                        </div>
                    </form>
                </Modal>
            </div>
